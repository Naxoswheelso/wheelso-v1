  ...
  try {
    ...
    if (data.length > 0) {
      EXTRAS = data.map(e => {
        ...
        return { id, code: e.code, name: e.name || def.name || id, priceLabel: perUnit ? t('ex_perDayItem', { price: price.toFixed(2) }) : t('ex_perDay', { price: price.toFixed(2) }), pricePerDay: price, perUnit, maxQty: e.max_qty || 4, icon: def.icon || ICON_EXTRA_DEFAULT, summary: e.description || def.summary || '', details: def.details || e.description || '' };
      });
    }
  } catch (err) {
