    ...
    if (data.length > 0) {
      EXTRAS = data.map(e => {
        ...
        const priceLabel = price === 0
          ? t('ex_free')
          : (perUnit ? t('ex_perDayItem', { price: price.toFixed(2) }) : t('ex_perDay', { price: price.toFixed(2) }));
        return { id, code: e.code, name: e.name || def.name || id, priceLabel, pricePerDay: price, perUnit, maxQty: e.max_qty || 4, icon: def.icon || ICON_EXTRA_DEFAULT, summary: e.description || def.summary || '', details: def.details || e.description || '' };
      });
    }
  } catch (err) {
...
function buildBookingPayload(formObj, afterHoursFee = 0) {
  ...
  const extrasArr = Object.entries(selectedExtras)
    ...
    .map(([id, qty]) => {
      ...
      return { code: (extra && extra.code) || id, quantity: qty };
    });
