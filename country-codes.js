// country-codes.js (vanilla / non-module)
// Attaches COUNTRY_CODES, PRIORITY_ISO, getOrderedCountries to window.
// Load via plain <script src="src/data/country-codes.js"></script>
// BEFORE checkout.js.
//
// Full list of ISO 3166-1 alpha-2 country codes with international phone prefixes.
// Sorted alphabetically by English country name.
// 239 entries, no duplicate ISO codes, all dial codes start with '+'.
...
(function (global) {
  ...
  // Top markets for Naxos tourism — surfaced at the top of the dropdown.
  var PRIORITY_ISO = ["GR", "DE", "FR", "IT", "GB", "NL", "US", "AT", "CH", "BE", "ES", "SE", "DK", "NO", "FI", "PL", "CZ", "IE", "AU", "CA"];

  function getOrderedCountries() {
    var prioritySet = {};
    for (var i = 0; i < PRIORITY_ISO.length; i++) prioritySet[PRIORITY_ISO[i]] = true;

    var priority = [];
    for (var j = 0; j < PRIORITY_ISO.length; j++) {
      for (var k = 0; k < COUNTRY_CODES.length; k++) {
        if (COUNTRY_CODES[k].iso === PRIORITY_ISO[j]) {
          priority.push(COUNTRY_CODES[k]);
          break;
        }
      }
    }
    var rest = [];
    for (var m = 0; m < COUNTRY_CODES.length; m++) {
      if (!prioritySet[COUNTRY_CODES[m].iso]) rest.push(COUNTRY_CODES[m]);
    }
    return { priority: priority, rest: rest };
  }

  global.COUNTRY_CODES = COUNTRY_CODES;
  global.PRIORITY_ISO = PRIORITY_ISO;
  global.getOrderedCountries = getOrderedCountries;
